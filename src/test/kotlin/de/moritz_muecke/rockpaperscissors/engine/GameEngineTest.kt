...
class GameEngineTest {
    ...
    fun `PlayerA wins when choosing ROCK while PlayerB chooses SCISSORS`() {
        ...
        assertEquals(engine.determineWinner(singleMatch), Some(players.first))
    }
    ...
    fun `PlayerA looses when choosing ROCK while PlayerB chooses PAPER`() {
        ...
        assertEquals(engine.determineWinner(singleMatch), Some(players.second))
    }
    ...
    fun `There is no winner when both PlayerA and PlayerB chooses the same action`() {
        ...
        assertEquals(engine.determineWinner(singleDrawMatch), None)
        assertEquals(engine.determineWinner(singleDrawMatch.copy(playerOneAction = Action.PAPER, playerTwoAction = Action.PAPER)), None)
        assertEquals(engine.determineWinner(singleDrawMatch.copy(playerOneAction = Action.SCISSOR, playerTwoAction = Action.SCISSOR)), None)
    }
